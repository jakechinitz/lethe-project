...
pub struct FeedQuery {
    ...
    #[serde(default)]
    pub sort: FeedSort,
    /// Cursor: ISO-8601 date ("YYYY-MM-DD") of the last item the client
    /// already has, paired with its thread id. Both must come together.
...
pub async fn get(
    ...
    let cursor = match (q.cursor_ts.as_deref(), q.cursor_id.as_deref()) {
        (Some(ts), Some(id)) => {
            ...
            let parsed = Date::parse(ts, fmt)
                .map_err(|_| AppError::BadRequest("cursor_ts must be YYYY-MM-DD"))?;
